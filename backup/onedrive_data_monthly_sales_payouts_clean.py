...
sheet_name = "Data_Sales_Payouts_Clean"  # ✅ 指定 Excel Sheet
table_name = "data_monthly_sales_payouts_clean"  # ✅ 指定 MySQL Table 名稱
...
def download_excel_from_onedrive(access_token, file_id):
    file_url = f"https://graph.microsoft.com/v1.0/users/{one_drive_config['user_email']}/drive/items/{file_id}/content"
...
def read_excel_sheet(excel_file, sheet_name):
    ...
    # 🔥 列出 Excel 內的所有 Sheet 名稱，確認名稱是否正確
...
def insert_data_to_mysql(df, connection, table_name):
    ...
    df = df.replace({pd.NA: None, "nan": None, "NaN": None, "": None, float("nan"): None})
